...
# At the top of improved_trainer_stable.py
from latentDLM_mmdit.diffusion_process import sample_t  # Add this import


class MultimodalDiffusionTrainer(nn.Module):
    def __init__(
        self, model, tokenizer, text_noise_schedule, dtype: torch.dtype, config
    ):
        super().__init__()
        self.model = model
    ...
    def count_params_by_group(self, mode="unconditional"):
        ...
        total_params = (
            sum(p.numel() for p in self.text_params)
    ...
    def verify_parameter_freezing(self, mode: str):
        ...
        if mode == "l2t":
            assert latent_trainable == 0, (
                f"L2T mode should have 0 trainable latent params, got {latent_trainable}"
            )
            assert text_trainable == text_total, (
                f"L2T mode should have all text params trainable"
            )
            print("✓ L2T verification passed!")

        elif mode == "t2l":
            assert text_trainable == 0, (
                f"T2L mode should have 0 trainable text params, got {text_trainable}"
            )
            assert latent_trainable == latent_total, (
                f"T2L mode should have all latent params trainable"
            )
            print("✓ T2L verification passed!")

        elif mode == "unconditional":
            assert text_trainable == text_total, (
                f"Unconditional mode should have all text params trainable"
            )
            assert latent_trainable == latent_total, (
                f"Unconditional mode should have all latent params trainable"
            )
    ...
    def _freeze_unneeded_params(self, mode: str):
        ...
        if self.current_step % 100 == 0:
            ...
            print(f"{'=' * 60}")
            print(
                f"Before freezing: {before_counts['total_trainable']:,} / {before_counts['total_params']:,} params trainable"
            )
            print(
                f"After freezing:  {after_counts['total_trainable']:,} / {after_counts['total_params']:,} params trainable"
            )
            ...
            if mode == "l2t":
                ...
                print(
                    f"✗ Latent params: {after_counts['latent_trainable']:,} trainable (frozen)"
                )
                print(
                    f"✓ Shared params: {after_counts['shared_trainable']:,} trainable"
                )
            elif mode == "t2l":
                print(
                    f"✗ Text params:   {after_counts['text_trainable']:,} trainable (frozen)"
                )
                print(
                    f"✓ Latent params: {after_counts['latent_trainable']:,} trainable"
                )
                print(
                    f"✓ Shared params: {after_counts['shared_trainable']:,} trainable"
                )
    ...
    def forward(self, batch, batch_idx=None, force_transitting: bool = False):
        ...
        if mode == "unconditional":
            ...
            latent_t = (
                self.latent_diffusion.sample_continuous_timesteps(
        ...
        # ===== MODE: Latent → Text =====
            ...
            text_t = torch.ones(batch_size, device=device)
            ...
            if latents is not None:
                ...
                p_uncond = getattr(self.config.training, "cfg_dropout_prob", 0.0)
        ...
        # ===== MODE: Text → Latent =====
            ...
            p_uncond = getattr(self.config.training, "cfg_dropout_prob", 0.0)
            if p_uncond > 0 and torch.rand(1).item() < p_uncond:
                noisy_input_ids = torch.full_like(
            ...
            latent_t = torch.ones(batch_size, device=device)
            if latents is not None:
                latents = latents.to(device=device, dtype=self.dtype)
                if latents.dim() == 3 and latents.shape[1] == 1:
                    latents = latents.squeeze(1)
        ...
        if compute_text_loss or compute_latent_loss:
            text_logits, latent_pred = self.model(
                ...
                latents=noisy_latents.unsqueeze(1)
        ...
        if (
            compute_latent_loss
            and latent_pred is not None
            and latent_target is not None
        ):
            # Ensure shapes match
            ...
            if (
                latent_pred.dim() == 2
                and latent_target.dim() == 3
                and latent_target.shape[1] == 1
            ):
            ...
            if torch.isnan(latent_pred).any() or torch.isinf(latent_pred).any():
                ...
                if torch.isnan(latent_loss) or torch.isinf(latent_loss):
                    ...
                    latent_loss_weight = getattr(
                        self.config.loss, "latent_loss_weight", 0.1
                    )
        ...
        with torch.no_grad():
            if compute_text_loss and text_logits is not None:
                ...
                if text_mask is not None and text_mask.any():
                    text_accuracy = (
                        (pred_tokens[text_mask] == text_target[text_mask])
                        .float()
                        .mean()
                        .item()
                    )
            ...
            if latents is not None:
                ...
                if compute_latent_loss and latent_pred is not None:
                    metrics["latent_pred_norm"] = float(
                        latent_pred.norm(dim=-1).mean().item()
                    )
...
class MultimodalDiffusionTrainer_new(nn.Module):
    ...
    def forward(self, batch, step=None):
        ...
        if mode == "unconditional":
            # Both modalities get noise - train both diffusion processes
            text_t = sample_t(
                self.config, batch_size, device=device
            )  # PROPER sampling for text
            latent_t = self.latent_diffusion.sample_continuous_timesteps(
                batch_size, device=device
            )  # PROPER sampling for latent
        ...
        #             compute_latent_loss = False
            ...
            # Use the PROPER sample_t function from diffusion_process.py
            text_t = sample_t(
                self.config, batch_size, device=device
            )  # This is the correct way!

            # Create partially masked input based on t
            noisy_input_ids = self.text_noise_schedule.sample_zt(input_ids, text_t)
            text_target = input_ids
            text_mask = (
                noisy_input_ids == self.mask_token_id
            )  # Only predict masked positions

            # Latent: clean conditioning (t=0 for latents)
        ...
        # ===== MODE: Text → Latent =====
            ...
            text_t = torch.zeros(batch_size, device=device)  # Clean text (t=0)
            noisy_input_ids = input_ids  # Clean text
            text_target = None
            text_mask = None

            # Latent: noisy (generate from text) - Use continuous diffusion sampling
            latent_t = self.latent_diffusion.sample_continuous_timesteps(
                batch_size, device=device
            )  # PROPER sampling for latent
            if latents is not None:
